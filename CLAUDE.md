...
`renderer/lib/markdown.ts` overrides the `fence` rule: a ` ```buttons ` fence is parsed by `shared/buttonBlock.ts` into `<button class="cmd-btn">` elements (one per line). A trailing ` // edit` on a line makes it paste-without-Enter (edit mode); otherwise the command is pasted and Enter is sent. A line whose trimmed content **starts with** `//` is rendered as a plain-text label (`<div class="cmd-text">`) instead of a button — so notes and buttons can interleave; `//` is positional (start = text, end = edit). `HelpPane` delegates clicks on `.cmd-btn` to `runCommand`; text rows have no `data-cmd` and are ignored.
