...
`renderer/lib/markdown.ts` overrides the `fence` rule: a ` ```buttons ` fence is parsed by `shared/buttonBlock.ts` into `<button class="cmd-btn">` elements (one per line). A trailing ` // edit` on a line makes it paste-without-Enter (edit mode); otherwise the command is pasted and Enter is sent. A line whose trimmed content **starts with** `//` is rendered as a plain-text label (`<div class="cmd-text">`) instead of a button — so notes and buttons can interleave; `//` is positional (start = text, end = edit). A line whose trimmed content **starts with** `#` is a shell-style **comment** — it stays in the md source only and is never rendered or collected (excluded from the quick-command dropdown too); like `//`, it is positional (`#` at line start = comment, ` # ` mid-line = label separator). `HelpPane` delegates clicks on `.cmd-btn` to `runCommand`; text rows have no `data-cmd` and are ignored.
...
- **The preload is sandboxed** (Electron 20+ defaults `sandbox: true`; `webPreferences` doesn't override it). In a sandboxed preload `require('electron')` exposes only `contextBridge`, `ipcRenderer`, `crashReporter`, `desktopCapturer`, `nativeImage`, `webFrame`, `webUtils` — **not** `clipboard`, `fs`, `path`, etc. So `import { clipboard } from 'electron'` in preload is `undefined` and any call throws (silently if guarded). Clipboard must run in the **main** process and be reached over IPC: `window.api.clipboard.readText()/writeText()` are `ipcRenderer.invoke` wrappers (`IPC.CLIPBOARD_READ`/`CLIPBOARD_WRITE`), hence **async** (return Promises) — callers `.then()` or fire-and-forget with `void`.
