...
# 命令                 → 生成一个按钮，按钮文字 = 命令本身
ls
# 命令 # 标签          → 按钮显示「标签」，运行时执行「命令」（简写）
# 命令 // edit         → 只粘贴到终端、不自动回车（编辑模式）
# 命令 # 标签 // edit  → 带标签的编辑模式
# 命令 ### label=标签; edit; tag=标记; tag-color=red
#                      → 结构化写法：### 后写属性（label/tag/tag-color 为值，edit 为布尔可裸写）
#                        多属性用 ; 分隔；值含 ; 或引号时用双引号包裹，如 tag="a; b"
#                        tag 会显示为按钮右端的小色标，tag-color 可选 red/amber/green/blue
